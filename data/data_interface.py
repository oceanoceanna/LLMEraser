...
class DInterface(pl.LightningDataModule):
    ...
    def __init__(self, 
        ...
        self.x_lr = kwargs['x_lr']
        self.x_init = kwargs['x_init']
        self.x_adjust = kwargs['x_adjust']
        self.x_iter = kwargs['x_iter']
        self.x_batch = kwargs['x_batch']
        ...
        if self.mode_type == 'normal':
            ...
            self.trainset = self.instancialize(stage='all')
            self.beforeset = self.instancialize(stage='before')
            self.atferset = self.instancialize(stage='after')
            self.valset = self.instancialize(stage='val')
            self.testset = self.instancialize(stage='test')
    ...
    def before_dataloader(self):
        return DataLoader(self.beforeset,
    ...
    def after_dataloader(self):
        return DataLoader(self.atferset,
                          batch_size=self.x_batch, 
                          num_workers=self.num_workers, 
                          shuffle=True,
                          drop_last=True,
