...
class MovielensData(data.Dataset):
    def __init__(self, seed =1234, ratio = 0.05,mode_type = 'normal', data_dir=r'data/ref/movielens',
                 stage='None',
                 cans_num=10,
                 sep=", ",
                 no_augment=True):
    ...
    def check_files(self):
        ...
        if self.mode_type == 'normal':
            ...
            if self.stage=='all':
                filename="/data3/dingcl/LLaRA/data/ref/movielens/"+str(self.mode_type)+"/train_data_"+str(self.mode_type) +'_'+ formatted_ratio + '.df'
                self.session_data = pd.read_pickle(filename)
            elif self.stage=='before':
                filename="/data3/dingcl/LLaRA/data/ref/movielens/"+str(self.mode_type)+"/before_data_"+str(self.mode_type) +'_'+ formatted_ratio + '.df'
                self.session_data = pd.read_pickle(filename)
            elif self.stage=='after':
                filename="/data3/dingcl/LLaRA/data/ref/movielens/"+str(self.mode_type)+"/after_data_"+str(self.mode_type) +'_'+ formatted_ratio + '.df'
                self.session_data = pd.read_pickle(filename)
            elif self.stage=='val':
                filename="Val_data.df"
                data_path=op.join(self.data_dir, filename)
...
class MovielensData_attack(data.Dataset):
    def __init__(self, seed =1234, ratio = 0.05,mode_type = 'normal', data_dir=r'data/ref/movielens',
                 stage=None,
                 cans_num=10,
                 sep=", ",
