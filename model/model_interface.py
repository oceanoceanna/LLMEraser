...
class MInterface(pl.LightningModule):
    ...
    def forward(self, batch):
        if self.influence == 100:
            ...
            targets = targets.masked_fill((batch["tokens"].token_type_ids == 0)[:,1:], -100).to(device)
            input_embeds = self.wrap_emb(batch).to(device)
    ...
    def get_optimizers_unlearning(self):
        if hasattr(self.hparams, 'weight_decay'):
            weight_decay = self.hparams.weight_decay
        else:
            weight_decay = 0
    ...
    def encode_items(self, seq):
        if self.influence == 100:
            ...
            if self.hparams.rec_embed=="SASRec":
                self.rec_model = self.rec_model.to(device)
    ...
    def embed_tokens(self, token_ids):
        if self.influence == 100:
            device = next(self.parameters()).device
            self.llama_model.base_model = self.llama_model.base_model.to(device)
    ...
    def wrap_emb(self, batch):
        if self.influence == 100:
            ...
            cans_item_embeds= self.encode_items(batch["cans"]).to(device)
